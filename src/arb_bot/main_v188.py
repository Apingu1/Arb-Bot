...
from .config_v188 import SettingsV188
...
from .runtime_v188 import BatchFirstMakerResearchSuiteV188, ParallelBatchFOKSuiteV188
from .storage_v186 import LowLatencyJsonlRecorderV186
from .strategy_v186 import ArbitrageEngineV186


def cli() -> None:
    # Keep legacy maker/taker controls disabled. Phase 1.8.8 adds only the
    # controlled BFOK freshness frontier while preserving the Phase 1.8.7
    # PFOK/BFOK/RAW comparison unchanged.
    for model in list(_runtime_controls_v18.DEFAULT_PROFILE):
        _runtime_controls_v18.DEFAULT_PROFILE[model] = set()

    _base.Settings = SettingsV188
    _base.ArbitrageEngine = ArbitrageEngineV186
    _base.JsonlRecorder = LowLatencyJsonlRecorderV186
    _base.MakerResearchSuite = BatchFirstMakerResearchSuiteV188
    _base.ShadowExecutor = InactiveTakerExecutor
    _base.DualFOKResearchSuite = ParallelBatchFOKSuiteV188
